...
- Скачиваемая сборка выкладывается релизом на GitHub: `gh release create sstp-plus-vX.Y.Z <apk> --repo HotJuice90/SSTP-`. Без `--repo` gh целится в апстрим, потому что тот тоже в remotes. APK переименовывать в `sstp-plus-X.Y.Z.apk`.
